...
2. **Trigger a real alert** — easiest is `AppHighErrorRate`, since
   ...
   test, run a short CPU stress container on the compose network, for
   example:

```bash
# install stress-ng locally in the container and run 4 CPU workers for 400s
docker run --rm --network observability-platform_obs-net polinux/stress-ng --cpu 4 --timeout 400s
```
